...
function updateTaskListFromBase() {
    ...
    for (const task of tasksArray.taskList) {
        if (task.priority === tasksArray.permittedPriorities['LOW']) {
            createTask(lowPriorityTasksWrapper, task.name, task.status);
        } else {
            createTask(highPriorityTasksWrapper, task.name, task.status);
        }
    }
}
...
function addTask(tasksWrapper) {
    return event => {
        ...
        let taskPriority = tasksWrapper.classList.contains('high-priority-tasks')
            ? tasksArray.permittedPriorities.HIGH
            : tasksArray.permittedPriorities.LOW;
        let taskInput = tasksWrapper.querySelector('.create-task-input');
        tasksArray.addTask(taskInput.value, tasksArray.permittedStatuses.TODO, taskPriority);
...
function changeTaskStatus() {
    return event => {
        ...
        if (element.classList.contains('task-checkbox')) {
            let taskText = element.nextElementSibling.textContent.trim();
            if (element.checked) {
                tasksArray.changeTaskStatus(taskText, tasksArray.permittedStatuses.DONE);
            } else {
                tasksArray.changeTaskStatus(taskText, tasksArray.permittedStatuses.TODO);
            }
            updateTaskListFromBase();
        }
    };
